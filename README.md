...
sample-group metadata are used for the single-end routing). The workflow
runs on older engines too — unknown `[workflow]` keys are ignored — but
the failure-email hook below only fires on **oxo-flow ≥ 0.17.0**. Release
binary (recommended):
